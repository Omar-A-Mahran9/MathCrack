...
class TestsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $coursesQuery = Course::with(['activeTests' => function ($query) {
                $query->orderBy('id');
            }])
        ...
        $coursesWithTests = $coursesQuery->get()->map(function ($course) use ($user) {
    ...
    public function mockTest(Request $request)
    {
        ...
        if ($courseId > 0) {
            $test = $this->findMockCandidate((clone $baseQuery)->where('course_id', $courseId), $testKey);
            ...
            $test = $this->findMockCandidate((clone $baseQuery)->where('course_id', $course->id), $testKey);
    ...
    public function show($id)
    {
        ...
        $activeAttempt = $allAttempts->whereIn('status', $this->activeStatuses())->first();
    ...
    public function start($id)
    {
        ...
        try {
            ...
            DB::rollBack();
            Log::error('Error starting test', $this->getLogContext($user, $test, $exception));

            return $this->jsonError('Error starting test: ' . $exception->getMessage(), 500);
        }
    }
    ...
    public function submit(Request $request, $id)
    {
        ...
        try {
            ...
            Log::error('Error submitting test', $this->getLogContext($user, $test, $exception));
    ...
    public function getRemainingTime($id)
    {
        ...
        return $this->jsonSuccess([
            'remaining_seconds' => max(0, $this->getRemainingTimeSeconds($studentTest)),
    ...
    public function report(Request $request, $id)
    {
        ...
        $previousAttempt = StudentTest::where('student_id', $user->id)
            ->where('test_id', $id)
        ...
        $questions = TestQuestion::where('test_id', $test->id)
            ->with(['answers' => function ($query) use ($studentTest) {
                $query->where('student_test_id', $studentTest->id);
            }])
            ->get();

        $previousAnswersMap = $this->buildPreviousAnswersMap($test, $previousAttempt);
        $topicReport = $this->buildTopicReport($questions, $previousAnswersMap);
        $recommendations = $this->buildRecommendations($topicReport);
        $chartLabels = $topicReport->pluck('topic')->values();
        $chartValues = $topicReport->pluck('percentage')->values();
        $previousChartValues = $topicReport->map(function ($topic) {
            return $topic['previous_percentage'] ?? 0;
        })->values();
        $finalSummary = $this->buildFinalSummary($topicReport);
    ...
    private function checkAttempts($user, $test)
    {
        ...
        if ($this->getActiveStudentTest($user, $test)) {
            return [
                'allowed' => false,
        ...
        if ($completedAttempts > 0 && $completedAttempts < (int) $test->max_attempts && $lastCompletedAttempt) {
    ...
    {
        return in_array($studentTest->status, $this->activeStatuses(), true);
    }

    private function isTimeUp($studentTest)
    {
        return $this->getRemainingTimeSeconds($studentTest) <= 0;
    }
    ...
    private function finalizeModuleTiming($studentTest, $test, $moduleNumber, $remainingSeconds)
    {
        ...
        $spent = max(0, min($moduleSeconds, $moduleSeconds - (int) $remainingSeconds));
    ...
    private function startNextModule($studentTest, $nextModule)
    {
        ...
        $startedAtColumn = 'part' . $nextModule . '_started_at';
        if ($this->hasColumn($studentTest, $startedAtColumn) && empty($studentTest->$startedAtColumn)) {
            $updates[$startedAtColumn] = now();
        }
    ...
    private function findMockCandidate($query, $testKey = 'mock')
    {
        ...
        if (Schema::hasColumn('tests', 'name')) {
            $candidate = (clone $query)
                ->where(function ($query) use ($testKey) {
                    $query->where('name', 'like', '%' . $testKey . '%');

                    if (strtolower($testKey) !== 'mock') {
                        $query->orWhere('name', 'like', '%mock%');
                    }
                })
                ->orderBy('id')
                ->first();
    ...
    private function buildPreviousAnswersMap($test, $previousAttempt)
    {
        if (!$previousAttempt) {
            return collect();
        }

        return TestQuestion::where('test_id', $test->id)
            ->with(['answers' => function ($query) use ($previousAttempt) {
            ...
            ->mapWithKeys(function ($question) {
                ...
                return [
                    $question->id => [
                        'topic' => $question->content ?: 'Uncategorized',
    ...
    private function buildTopicReport($questions, $previousAnswersMap)
    {
        return $questions->groupBy(function ($question) {
            return $question->content ?: 'Uncategorized';
            ...
            foreach ($topicQuestions as $question) {
                ...
                if ($previousAnswersMap->has($question->id) && $previousAnswersMap[$question->id]['is_answered']) {
                    $previousAnswered++;

                    if ($previousAnswersMap[$question->id]['is_correct']) {
                        $previousCorrect++;
                    }
                }
            }
            ...
            $difficultyBreakdown = $this->buildDifficultyBreakdown($topicQuestions);
            ...
            return [
                ...
                'level' => $this->topicLevel($percentage),
                'difficulty_breakdown' => $difficultyBreakdown,
                'study_recommendation' => $this->studyRecommendation($topicName, $percentage),
            ];
        })->values();
    }

    private function buildDifficultyBreakdown($topicQuestions)
    {
        return $topicQuestions->groupBy(function ($question) {
            ...
            foreach ($difficultyQuestions as $question) {
                $answer = $question->answers->first();
                $isAnswered = (bool) $answer;
            ...
            return [
                ...
                'unanswered' => $unanswered,
                'accuracy' => $answered > 0 ? round(($correct / $answered) * 100, 1) : 0,
                'coverage' => $total > 0 ? round(($answered / $total) * 100, 1) : 0,
            ];
        })->values();
    }

    private function topicLevel($percentage)
    {
        if ($percentage < 40) {
            return 'Weak';
        }
    ...
    private function studyRecommendation($topicName, $percentage)
    {
        if ($percentage >= 85) {
            return "You are exam-ready in {$topicName}.";
        }

        if ($percentage >= 65) {
            return "You are doing well in {$topicName}, but you still need targeted practice.";
        }

        if ($percentage >= 40) {
            return "You need more structured revision in {$topicName}.";
        }

        return "You need serious work in {$topicName}. Start with basics and easy questions.";
    }

    private function buildRecommendations($topicReport)
    {
        $recommendations = [];
        $weakTopics = $topicReport->filter(fn ($topic) => $topic['percentage'] < 50)->pluck('topic')->values();
        $strongTopics = $topicReport->filter(fn ($topic) => $topic['percentage'] >= 80)->pluck('topic')->values();
    ...
    private function buildFinalSummary($topicReport)
    {
        $finalSummary = [];

        foreach ($topicReport as $topic) {
            if ($topic['percentage'] >= 85) {
                $finalSummary[] = "You are exam-ready in {$topic['topic']}.";
            } elseif ($topic['percentage'] < 50) {
                $finalSummary[] = "You need more work in {$topic['topic']}.";
            }
        }
